...
def process(pn, args):
    ...
    if args.v_type in ["5", "6", "7"]:
        ...
        model = timm.create_model(vit_name, pretrained=False, num_classes=num_classes, drop_rate=args.vit_drop_rate, 
                                  attn_drop_rate=args.vit_attn_drop_rate, drop_path_rate=args.vit_drop_path_rate, 
                                  pos_drop_rate=args.vit_pos_drop_rate, proj_drop_rate=args.vit_proj_drop_rate)
    ...
    for epoch in range(start_epoch, args.total_epochs):
        ...
        for layer in model.module.modules():
            if isinstance(layer, PolyNorm):
                ...
                layer.epoch_test_var_mean = 0
                layer.epoch_test_var_sum = 0
                layer.epoch_test_var_mean_count = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vit_pos_drop_rate", type=float, default=0.1)
    parser.add_argument("--vit_proj_drop_rate", type=float, default=0.1)
    ...
    parser.add_argument('--poly_weight_inits', nargs='+', type=float, default=[0, 1, 0], help='relu_poly weights initial values')
    parser.add_argument('--poly_weight_factors', nargs='+', type=float, default=[0.1, 1, 1], help='adjust the learning rate of the three weights in relu_poly')
