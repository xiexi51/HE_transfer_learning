...
class custom_relu(nn.Module):
    ...
    def forward(self, x):
        ...
        assert self.custom_settings.relu_type == "channel"
...
class BasicBlockAvgCustom(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, custom_settings, out_features):
        ...
        self.conv1 = nn.Conv2d(custom_settings, in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.conv2 = nn.Conv2d(custom_settings, planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            ...
            self.shortcut = nn.Sequential(
                nn.Conv2d(custom_settings, in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False),
                self.shortcut_norm
            )
...
class ResNetAvgCustom(nn.Module):
    def __init__(self, block, num_blocks, num_classes, custom_settings, if_wide):
        ...
        self.in_planes = 64
        self.conv1 = nn.Conv2d(custom_settings, 3, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
def ResNet18AvgCustom(custom_settings, if_wide):
    assert False 
    assert custom_settings.norm_type == "my_layernorm"
