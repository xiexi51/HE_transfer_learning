...
class PolyNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 对除最后一维外的所有维度取平均，得到标量（保持与原实现一致的标量 running_var_mean）
        reduce_dims = tuple(range(var.dim() - 1))  # 所有非最后一维
        var_mean = var.mean(dim=reduce_dims).squeeze()
